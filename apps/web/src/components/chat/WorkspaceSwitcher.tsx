...
export function WorkspaceSwitcher({ rail }: { rail?: boolean }) {
    ...
    return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            className="h-9 w-9 rounded-[var(--radius-md)] bg-gradient-to-br from-[var(--primary)] to-[var(--accent)] flex items-center justify-center text-[var(--primary-foreground)] text-xs font-bold"
            title={active ? `${active.name} — switch workspace` : "Select workspace"}
          >
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          className="flex min-w-0 flex-1 items-center gap-2 rounded-[var(--radius-md)] px-2 py-1.5 -mx-1 text-left hover:bg-sidebar-foreground/5 transition-colors"
        ...
        >
          ...
          <span className="flex-1 min-w-0">
            ...
            <span className="block text-[11px] text-sidebar-foreground/40">{workspaces.length} workspace{workspaces.length === 1 ? "" : "s"}</span>
          </span>
